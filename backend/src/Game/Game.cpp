    ...
    for (int i = 0; i < grid_size; ++i) {
        for (int j = 0; j < grid_size; ++j) {
            try {
                ...
                std::cout << "jaki� b��d z variantami" << std::endl;
            }
        }
    }
...
int Game::countMovesToWin(sign player, std::unordered_map<std::string, int>& cache) {
    ...
    // Sprawd�, czy wynik dla tego stanu planszy jest ju� w pami�ci podr�cznej
    ...
    // Sprawd�, czy gracz wygra� w bie��cym stanie planszy
    if (game.is_won().win && game.is_won().sign_ == player) {
        return 0;
    }
    ...
    for (size_t i = 0; i < board->getGrid().size(); ++i) {
        for (size_t j = 0; j < board->getGrid()[i].size(); ++j) {
            if (std::get<sign>(board->getGrid()[i][j]) == sign::nothing) {
                ...
                // Stw�rz now� instancj� Game, korzystaj�c z aktualnej planszy
                Game game(board);

                // Rekurencyjne wywo�anie funkcji dla nast�pnego gracza
                int moves = game.countMovesToWin(player, cache);

                // Przywr�� poprzedni stan planszy
                board->getGrid()[i][j] = originalValue;

                // Zaktualizuj minimaln� liczb� ruch�w
                if (moves >= 0 && moves < minMoves) {
                    minMoves = moves;
                }
            }
    ...
    // Dodaj ruch bie��cego gracza do minimalnej liczby ruch�w
    int result;
    if (minMoves != std::numeric_limits<int>::max()) {
        result = minMoves + 1;
    }
    else {
        result = -1; // Brak mo�liwych ruch�w
    }

    // Zapisz wynik dla tego stanu planszy w pami�ci podr�cznej
